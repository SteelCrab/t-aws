...
run_for_target() {
  ...
  cargo clippy --fix --manifest-path "$MANIFEST_PATH" --bin "$bin" -p "$package" $extra_fix_flags

  echo "=== cargo clippy ==="
  # shellcheck disable=SC2086
  cargo clippy --manifest-path "$MANIFEST_PATH" --bin "$bin" -p "$package" $extra_check_flags

  if [[ "$RUN_DEADCODE" == "1" ]]; then
    echo "=== cargo clippy (dead-code) ==="
    # shellcheck disable=SC2086
    cargo clippy --manifest-path "$MANIFEST_PATH" --bin "$bin" -p "$package" $extra_check_flags -- -W dead-code
  fi
}
...
if [[ "$RUN_TESTS" == "1" ]]; then
  echo "=== cargo test ==="
  cargo test --manifest-path "$MANIFEST_PATH" --all-targets --locked
fi
